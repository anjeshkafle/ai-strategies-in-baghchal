        ...
        if abs(from_x - to_x) == 1 and abs(from_y - to_y) == 1:
            ...
            if is_outer_layer(from_x, from_y):
                is_second_or_fourth_node = (
                    (from_x in (0, 4) and from_y in (1, 3)) or
                    (from_y in (0, 4) and from_x in (1, 3))
                )
    ...
    if piece["type"] == "TIGER":
        ...
        for dx, dy in directions:
            ...
            if (
                ...
                is_valid_connection(x, y, mid_x, mid_y) and
                is_valid_connection(mid_x, mid_y, jump_x, jump_y)
            ):
                moves.append({
                    "x": jump_x,
                    "y": jump_y,
                ...
                if board[y][x] and board[y][x]["type"] == piece_type:
                    ...
                    for move in piece_moves:
                        moves.append({
                            ...
                            "capture": move.get("capturedPiece")
                        })
